...
class Inputs:
    ...
    def read_number(self, name):
        for key in list(self.inputs[name].keys()):
            if(key[0] != "w"):
                continue
            wire_name = self.inputs[name][key]
            if self.inputs.get(name) is None:
                raise InvalidInputNameException(f"{name} is not declared in inputs")
            ...
            if self.inputs[name].get("created", False):
                ...
                # wires = ''.join(list(self.inputs[name].keys())).count('wire')
                # for i in range(wires):
                    # wire_name = self.inputs[name][str("wire" + str(i) )]
